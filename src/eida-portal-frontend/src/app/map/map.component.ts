...
export class MapComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._map.on("click", function(e) {
      let feature = e.map.forEachFeatureAtPixel(e.pixel, function(feature) {
        return feature;
      });
      if (feature) {
    ...
    this._stationsService.selectedStations.subscribe(s =>
      this.updateStationsMap(s)
    );

    this._stationsService.focuedStation.subscribe(s => this.focusStation(s));

    this._eventsService.focusedEvent.subscribe(e => this.focusEvent(e));

    this._eventsService.selectedEvents.subscribe(s => this.updateEventsMap(s));
    ...
    draw.on("boxend", () => {
      ...
      this._map.getView().fit(e, { duration: 1000 });

      // Send the coordinates to the map service so other components
      // which subscribe to it can be notified
      this._mapService.updateDragBoxCoordinatesByArray(coordSN.concat(coordEW));
    });

    this._mapService.mapResizeTrigger.subscribe(e => this.updateMapSize(e));

    this._mapService.mapZoomResetTrigger.subscribe(e => this.resetMapZoom());
  }
  ...
  updateStationsMap(stations: FdsnStationExt[]) {
    ...
    for (let s of stations) {
      var point = new ol.Feature({
        ...
        name: `<b>Network:</b> ${s.station_network_code}<br><b>Station:</b> ${
          s.station_code
        }<br><b>Name:</b> ${s.station_site_name}`
      });

      point.setStyle(
        new ol.style.Style({
          image: new ol.style.Icon({
            src: s.station_selected
